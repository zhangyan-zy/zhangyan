...
<!--      <el-input v-model="dataForm.price" placeholder="优惠券金额"></el-input>-->
      <el-input-number v-model="dataForm.price" :precision="0" :step="1"></el-input-number>
    </el-form-item>
    </el-form>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/generator/coupon/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.coupon.name
                this.dataForm.createTime = data.coupon.createTime
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/coupon/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
